...
#
# SOCKS5 CONNECT proxy: fetch a payload through the proxy to an IPv6-literal
# target ([::1], SOCKS5 ATYP=IPv6) and verify the bytes are intact, on every
...
for loop in epoll io_uring; do
	...
	curl -s --max-time 20 --proxy "socks5h://[::1]:$pp" \
		"http://[::1]:$hp/payload.bin" -o "$WORK/out.bin" \
		|| fail "[$loop] curl via SOCKS5 to IPv6 target failed"
	assert_files_equal "$WORK/payload.bin" "$WORK/out.bin" \
		"[$loop] SOCKS5 proxy corrupted the IPv6-target payload"
